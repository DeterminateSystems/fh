...
This commit's fh binaries can be fetched from:

Intel macOS:

\`\`\`
curl --output fh --proto '=https' --tlsv1.2 -sSf -L https://install.determinate.systems/fh/rev/$GIT_ISH/X64-macOS
\`\`\`

x86_64 Linux:

\`\`\`
curl --output fh --proto '=https' --tlsv1.2 -sSf -L https://install.determinate.systems/fh/rev/$GIT_ISH/X64-Linux
\`\`\`
...
\`\`\`
curl --output fh --proto '=https' --tlsv1.2 -sSf -L https://install.determinate.systems/fh/${TYPE}/${TYPE_ID}/X64-macOS
\`\`\`

x86_64 Linux:

\`\`\`
curl --output fh --proto '=https' --tlsv1.2 -sSf -L https://install.determinate.systems/fh/${TYPE}/${TYPE_ID}/X64-Linux
\`\`\`
EOF
