...
impl Handler for Python {
    fn handle(project: &Project, flake: &mut Flake) {
        if project.has_one_of(&["setup.py", "requirements.txt"]) && Prompt::bool("This seems to be a Python project. Would you like to initialize your flake with built-in Python dependencies?") {
            ...
            flake.dev_shell_packages.push(format!("python{python_version_attr}"));
